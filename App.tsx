        ...
        <div className="bg-[#FDFBF7] border-4 border-black shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] max-w-3xl w-full relative max-h-[85vh] overflow-y-auto">
          <div className="bg-yellow-400 p-4 border-b-4 border-black flex justify-between items-center sticky top-0 z-10">
            <h2 className="text-2xl font-black uppercase tracking-tight flex items-center gap-2">
              <BookOpen size={24} /> Character Dossier
            </h2>
            ...
            {EDUCATIONAL_MODULES.map((module, i) => (
              <div key={i} className="flex gap-4 items-start">
                <div className="text-4xl bg-white border-2 border-black w-16 h-16 flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] shrink-0">
                  {module.icon === 'character' ? <User size={32} /> :
                    module.icon === 'ai' ? <Zap size={32} /> :
                      <Info size={32} />}
                </div>
                <div>
          ...
          <div className="p-4 bg-gray-100 border-t-4 border-black text-center">
            <Button onClick={() => setShowHelp(false)} variant="secondary" className="text-sm">Close Dossier</Button>
          </div>
        </div>
      </div>
    );
  ...
  const renderIntro = () => (
    <div className="min-h-screen flex flex-col relative">
        ...
        <div className="max-w-4xl w-full">
          <div className="bg-white border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-8 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h1 className="text-6xl font-black mb-2 tracking-tighter uppercase">The Panthropic AGI Files</h1>
              <div className="text-4xl font-black mb-6 tracking-tighter uppercase text-[#DC2626]">BSEV Edition</div>
      ...
      <div className="w-full p-6 flex justify-between text-xs font-bold text-gray-400 uppercase tracking-widest bg-white border-t-2 border-gray-200">
        <div>Copyright: FinvesT, IIMT @2026</div>
          ...
          <Card title="Market Intelligence" accentColor="bg-blue-800" className="flex-grow flex flex-col min-h-[400px]">
            <div className="flex-1 overflow-y-auto pr-4">
              ...
              <div className="grid grid-cols-4 gap-3 mb-8">
                {[
                  { label: 'Fear Index', val: currentScenario.macro.fearIndex, icon: <Zap size={14} />, color: 'bg-red-100', text: 'text-red-900', border: 'border-red-900' },
                  { label: 'Sentiment', val: currentScenario.macro.sentiment, icon: <Globe size={14} />, color: 'bg-yellow-100', text: 'text-yellow-900', border: 'border-yellow-900' },
                  { label: 'Valuation', val: currentScenario.macro.valuation, icon: <Activity size={14} />, color: 'bg-blue-100', text: 'text-blue-900', border: 'border-blue-900' },
                  { label: 'Compute', val: currentScenario.macro.computeDemand, icon: <Percent size={14} />, color: 'bg-purple-100', text: 'text-purple-900', border: 'border-purple-900' }
                ].map((m, i) => (
        ...
        <div className="bg-[#FDFBF7] border-4 border-black shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] max-w-5xl w-full relative my-8">
          <div className={`w-full p-6 border-b-4 border-black flex justify-between items-center ${isGain ? 'bg-green-100' : 'bg-red-100'}`}>
            <div>
              ...
              <p className="font-bold text-gray-600 text-sm uppercase tracking-widest">Round Statement</p>
            </div>
          ...
          <div className="p-6 bg-gray-50 border-t-4 border-black flex gap-4">
            <Button fullWidth onClick={nextRound} className="text-xl py-4 flex items-center justify-center gap-2">
              {gameState.currentRound >= MAX_ROUNDS ? "View Final Evaluation" : "Advance to Next Round"} <ArrowRight />
            </Button>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-[#FDFBF7] p-8">
        <div className="max-w-6xl mx-auto">
          <Card title="BSEV AGI Saga — Performance Evaluation" className="text-center p-10">
