...
export const AssetControl: React.FC<AssetControlProps> = ({
  ...
  // Selling is maxed out if we hit regulatory limit (and we have enough assets to hit it)
    ...
    <div className="mb-5 last:mb-0">
      ...
      <div className="relative h-12 flex items-center group">
        ...
        <div className="absolute w-full h-3 bg-gray-200 border-2 border-black rounded-full overflow-hidden">
           ...
           {/* Sell Limit Restriction Pattern (Left Side) */}
           {/* Shows blocked area if we have more asset than we can sell */}
           {currentValue > maxMove && (
             <div 
               className="absolute top-0 bottom-0 z-0 opacity-80" 
               style={{
                 ...blockedPatternStyle,
                 left: 0,
                 right: `calc(50% + ${(maxMove / sliderRange) * 50}%)`
               }}
             />
           )}

           {/* Buy Limit Restriction Pattern (Right Side) */}
           {/* Shows blocked area if cash is insufficient to buy up to the full slider range */}
           {effectiveMaxBuy < sliderRange && (
             <div 
               className="absolute top-0 bottom-0 z-0 opacity-80" 
               style={{
                 ...blockedPatternStyle,
                 left: `calc(50% + ${(effectiveMaxBuy / sliderRange) * 50}%)`,
                 right: 0
               }}
             />
           )}
      ...
      <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
        <span className="flex items-center gap-1">
          <Lock size={10}/> Max Sell: ₹{effectiveMaxSell.toFixed(0)} Cr
        </span>
        <span className="flex items-center gap-1 text-right">
           {isBuyMaxed ? <span className="text-red-600">Cash Limit Hit</span> : `Cash Avail: ₹${effectiveMaxBuy.toFixed(0)} Cr`} 
        </span>
      </div>
    </div>
  );
